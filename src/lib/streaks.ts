// Streak + daily-progress storage utilities (per-user via pstore).

import { pget, pset } from "./pstore";
...
export const loadDaily = (): Record<string, DayLog> => {
  try {
    return JSON.parse(pget("p0_daily") ?? "{}");
  } catch {
    return {};
  }
};
export const saveDaily = (m: Record<string, DayLog>) =>
  pset("p0_daily", JSON.stringify(m));

export const loadStreak = (): StreakState => {
  try {
    return (
      JSON.parse(pget("p0_streak") ?? "null") ?? {
        current: 0,
        longest: 0,
        lastFullDate: null,
      }
...
export const saveStreak = (s: StreakState) =>
  pset("p0_streak", JSON.stringify(s));
...
export const bumpStreakIfComplete = (
  ...
  if (completionPercent(log) < 100) return prev;
