...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around bg-sidebar/90 backdrop-blur-xl border-t border-border/60 px-2 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        {items.map((item) => {
          const active = path === item.to || path.startsWith(item.to + "/");
          return (
            <Link
